...
void Graph<T>::dijsktraSSSP(T src, map<T,float> &dist, map<T,T> &prev)
{
    ...
    dist[src] = 0;//value is being modified
    s.insert(make_pair(0,src));//after making pair it is inserted into the s
...
{   system("notify-send -t 5000 -i /home/nightwarrior-xxx/Documents/MetroWorksDS/train1.png \"Metro Works\"");
    ...
    //system("echo \"\e[96m\"");
    getline(cin,deststn);
    bool res=Metro.printAdj(sourcestn,deststn);//It will check whether Source and Destination name is corrct or not 
    if(res==false)
    {
